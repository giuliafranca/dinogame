...
class MultiDinoGame:
    ...
    def reset(self):
        ...
        self.alive_players = self.player_dinos[:]  # Todos os dinossauros começam vivos
...
def action_list(state):
    num_dinos = len(state) // 3  # Considerando que cada obstáculo tem 3 parâmetros (posição x, posição y e altura)
    actions = []

    for i in range(num_dinos):
        # Coleta informações do obstáculo mais próximo para este dinossauro
        obstacle_x = state[i * 3]
        obstacle_y = state[i * 3 + 1]
        obstacle_height = state[i * 3 + 2]
        
        # Lógica para pular ou abaixar
        if obstacle_x < 0.15:  # Se o obstáculo está muito próximo (15% da largura da tela)
            if obstacle_y > 0.8 and obstacle_height < 0.15:  # Ptera voando baixo
                action = ACTION_DOWN
            else:
                action = ACTION_UP  # Pular se é um cacto ou Ptera alto
        else:
            action = ACTION_FORWARD  # Continuar correndo se o obstáculo não está perto
